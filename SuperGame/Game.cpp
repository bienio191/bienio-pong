...
sf::Vector2f Game::calculateVelocity(Paddle& paddle, Ball& ball) {
	...
	if (isInBounds(normValue, (float) 0.8, (float)1)) {
		...
		return sf::Vector2f(2.5, -5.0);
		//2
	}
	else if (isInBounds(normValue, (float) 0.6, (float) 0.8)) {
		//cout << "isInBounds(normValue, 0.6, 0.8)" << endl;
		return sf::Vector2f(3, -4.5);
		//3
	}
	else if (isInBounds(normValue, (float) 0.2, (float) 0.6)) {
		//cout << "isInBounds(normValue, 0.2, 0.6)" << endl;
		return sf::Vector2f(4.5, -3);
		//4
	}
	else if (isInBounds(normValue, (float)-0.2, (float) 0.2)) {
		//cout << "isInBounds(normValue, -0.2, 0.2)" << endl;
		return sf::Vector2f(5.5, 0);
		//5
	}
	else if (isInBounds(normValue, (float)-0.6, (float)-0.2)) {
		//cout << "isInBounds(normValue, -0.6, -0.2)" << endl;
		return sf::Vector2f(4.5, 3);
		//6
	}
